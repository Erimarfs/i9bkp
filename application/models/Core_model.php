...
class Core_model extends CI_Controller{
    ...
    public function insert($tabela = null, $data = null, $get_last_id = null){
        if($tabela && $this->db->table_exists($tabela) && is_array($data)){
            ...
            if($this->db->affected_rows() > 0){
              $this->session->set_flashdata('sucesso','Dados incluidos com sucesso');
            }else{
                $this->session->set_flashdata('erro','Não foi possivel incluir os dados');
            }
        } else {
            return false;
        }
    ...
    public function update($tabela = null, $data = null, $condicoes = null){
        if($tabela && $this->db->table_exists($tabela) && is_array($data) && is_array($condicoes)){
            if($this->db->update($tabela, $data, $condicoes)){
                ...
                $this->session->set_flashdata('erro','Não foi possivel alterar os dados');
            }
        }else{
            return false;
        }
